...
const PERSONALIZATION_OFF = true;

/**
 * Pure mapper: a resolved preset (+ optional tone) → the cloud skill payload.
 * Tone rides in `args.tone` as a length-1 list; omitted entirely when null or
 * undefined so the server receives the preset with no personalization tone.
 */
export function buildPresetSkill(
  preset: string,
  tone?: string | null,
): DictationSkill {
  const skill: DictationSkill = { preset };
...
async function builtInToneByPreset(): Promise<Map<string, string>> {
  ...
  try {
    ...
    // the preset alone rather than failing the transcription.
    logger.transcription.warn("Skill tone lookup failed; using preset only", {
