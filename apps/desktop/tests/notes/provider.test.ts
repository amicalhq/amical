...
describe("Markdown editor persistence order", () => {
  it("coalesces debounce and flushes synchronously before unmount/reopen", () => {
    const provider = new NoteSyncProvider(1, api);
    ...
    const reopened = new NoteSyncProvider(1, api);
    expect(reopened.body.markdown).toBe("newer");
    reopened.destroy();
  });
  it("lets the last local save win across windows", () => {
    const first = new NoteSyncProvider(1, api),
      second = new NoteSyncProvider(1, api);
    const status = vi.fn();
  ...
  it("refreshes an idle window without an echo save", () => {
    ...
    second.queue("remote");
    second.flush();
    listeners.forEach((fn) => fn({ noteId: 1 }));
  ...
  it("never resurrects a deleted note, even on close", () => {
    ...
    provider.queue("late");
    deleted = true;
    listeners.forEach((fn) => fn({ noteId: 1, deleted: true }));
